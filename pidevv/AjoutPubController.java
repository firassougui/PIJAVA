...
public class AjoutPubController implements Initializable {
    ...
    private void ajouter(ActionEvent event) {
        ...
        Date date = java.sql.Date.valueOf(dd);
               ...
               .title("Alert").text(" FindJob n'autorise pas ces termes").graphic(null).hideAfter(javafx.util.Duration.seconds(5))
        ...
        if(description.getText().isEmpty())
    {
       ...
       Notifications notificationBuilder = Notifications.create()
               .title("Succes").text("Votre Publication est ajoutèe avec succès").graphic(null).hideAfter(javafx.util.Duration.seconds(5))
               .position(Pos.CENTER_LEFT)
               .onAction(new EventHandler<ActionEvent>(){
                   public void handle(ActionEvent event)
                   {
